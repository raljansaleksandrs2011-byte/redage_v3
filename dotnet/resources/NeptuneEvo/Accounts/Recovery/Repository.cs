    ...
    class Repository
    {
        ...
        public static void SendEmail(ExtPlayer player, string login)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"A message with a password recovery code has been successfully sent to {Generate.ObfuscateEmail(auntificationData.Email)}.", 5000);
        ...
        public static async void RecoveryPassword(ExtPlayer player, string code)
        {
            try
            {
                ...
                if (code == sessionData.RecoveryCode)
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "You can also change it right now from within the game by entering /password followed by a space and your new desired password. Example: [/password 123] without the brackets.", 10000);
                }
                else
                {
